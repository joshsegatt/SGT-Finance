...
export default function BankCallbackPage() {
  ...
  useEffect(() => {
    ...
    const tlError = searchParams.get("error");

    if (tlError) {
      const desc = searchParams.get("error_description") ?? tlError;
      setError(desc);
      setStatus("error");
      return;
    }
    ...
    fetch(`/api/banks/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`)
      .then(async (res) => {
            ...
            <>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1.5">
                  Fetching accounts and balances from TrueLayer…
                </p>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">
                  <span className="font-medium text-foreground">{result.institution}</span> — {result.accountsCreated} account
