...
export default async function SettingsPage() {
  ...
  const [user, accounts, t, dbUser] = await Promise.all([
    getUserById(session.user.id),
    getAccounts(),
    getTranslations("Settings"),
    import("@/lib/db").then(({ db }) => db.user.findUnique({ where: { id: session.user.id }, select: { plan: true, stripeCurrentPeriodEnd: true } })),
  ]);
                  ...
                  <div key={account.id} className="flex items-center gap-3 px-6 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">
                        {account.entity?.name ?? "—"} · {account.accountType}
                      </p>
                    </div>
                ...
                <div className="w-10 h-10 rounded-full overflow-hidden bg-primary/10 flex items-center justify-center shrink-0 ring-1 ring-border">
                  {user.image ? (
                    ...
                    <span className="text-sm font-bold text-primary">
                      {(user.name ?? "U").split(" ").slice(0, 2).map((w) => w[0]?.toUpperCase()).join("")}
                    </span>
                  )}
                </div>
          ...
          <div className="bg-card rounded-xl border border-border panel-shadow overflow-hidden">
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              {dbUser?.stripeCurrentPeriodEnd && (
                <div className="flex items-center justify-between border-t border-border/30 pt-2.5">
                  ...
                  <span className="text-xs font-medium text-foreground">
                    {new Date(dbUser.stripeCurrentPeriodEnd).toLocaleDateString("pt-PT")}
                  </span>
                </div>
              )}
              <Link
              ...
              >
                ...
                {dbUser?.plan === "FREE" ? "Fazer upgrade" : "Gerir subscrição"}
              </Link>
            </div>
          </div>
